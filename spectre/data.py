...
# ZARR_FILE = "/scratch/pgrosjean/aligned_codex_registered_matched_channels_updated.zarr"
ZARR_FILE = "/scratch/pgrosjean/aligned_codex_registered_matched_channels_updated_normalized.zarr"
...
# DataModules
class HEProtDataModule(pl.LightningDataModule):
...
class HEtoCODEXDataModule(pl.LightningDataModule):
    def __init__(self,
                 train_config: TrainConfig):
        super().__init__()
        self.train_config = train_config
...
class SpectreDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.channel_num is not None:
            codex = codex[self.channel_num].unsqueeze(0)
        return he, codex
...
class SpectreDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        train_ds = SpectreDataset(codex_keys=['aligned_codex_1', 'aligned_codex_2', 'aligned_codex_3'], channel_num=self.channel_num)
        return generate_dataloader(self.train_config, train_ds, shuffle=True)
    
    def val_dataloader(self):
        val_ds = SpectreDataset(codex_keys=['aligned_codex_5'], channel_num=self.channel_num)
        return generate_dataloader(self.train_config, val_ds, shuffle=True)
    

class SpectreDatasetSmall(torch.utils.data.Dataset):
    def __init__(self,
                 codex_keys: Union[List[str],None]=None,
                 channel_num: None = None):
        self.zarr_root = zarr.open(ZARR_FILE, mode='r')
        self.codex_keys = list(self.zarr_root.keys())
        self.codex_keys = [key for key in self.codex_keys if key != 'aligned_codex_7']
        if codex_keys is not None:
            ...
            self.codex_keys = [key for key in self.codex_keys if key in codex_keys]
        self.codex_indices = self._get_codex_indices()
        self.codex_indices = self.codex_indices[:1]
        # self.threshold_dict, self.stats_dict = self._calculate_otsu_thresholds_per_image()
        self.he_normalizer = Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225))
        self.channel_num = channel_num
...
class SpectreDataSmallModule(pl.LightningDataModule):
    def __init__(self,
                 train_config: TrainConfig,
                 channel_num = None):
        super().__init__()
        if channel_num is not None:
            assert channel_num in np.arange(13), "Invalid channel_num"
        self.channel_num = channel_num
        self.train_config = train_config

    def train_dataloader(self):
        train_ds = SpectreDatasetSmall(codex_keys=['aligned_codex_1', 'aligned_codex_2', 'aligned_codex_3'], channel_num=self.channel_num)
        return generate_dataloader(self.train_config, train_ds, shuffle=False)
    
    def val_dataloader(self):
        val_ds = SpectreDatasetSmall(codex_keys=['aligned_codex_5'], channel_num=self.channel_num)
        return generate_dataloader(self.train_config, val_ds, shuffle=False)
